...
if ( $setting['icon_link']['url'] ) {
	$target      = $settings['icon_link']['is_external'] ? ' target="_blank"' : '';
	$nofollow    = $settings['icon_link']['nofollow'] ? ' rel="nofollow"' : '';
...
switch ( $setting['icon'] ) {
	case 'upload_icon':
		if ( $setting['icon_upload'] ) {
			...
			$alt         = isset( $setting['title'] ) ? $setting['title'] : esc_attr__( 'Icon', 'course-builder' );
