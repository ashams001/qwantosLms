...
if ( $setting['list_courses'] === 'category' ) {
	if ( $setting['cat_courses'] ) {
		$tax_query_value = array(
			array(
				'taxonomy' => 'course_category',
				'field'    => 'id',
				'terms'    => $setting['cat_courses'],
			)
		);
	}
} else {
...
$args_list_courses = array(
	'posts_per_page' => $setting['number_courses'],
	'post_type'      => 'lp_course',
	'post_status'    => 'publish',
	'tax_query'      => $tax_query_value,
);

if ( $setting['list_courses'] === 'latest' ) {
	$args_list_courses['orderby'] = 'date';
}

if ( $setting['list_courses'] == 'popular' ) {
	$args_list_courses['post__in'] = lp_get_courses_popular();
}

// Get featured courses
if ( $setting['featured_courses'] == 'yes' ) {
...
$button_link = $setting['button_link']['url'];
