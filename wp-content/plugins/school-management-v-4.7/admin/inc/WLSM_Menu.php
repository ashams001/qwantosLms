...
class WLSM_Menu {
		...
		if ( WLSM_Helper::lm_valid() ) {
			...
			if ( $current_school = $user_info['current_school'] ) {
				...
				if ( in_array( $role, array_keys( WLSM_M_Role::get_roles() ) ) ) {
					...
					// Academic - Group.
					if ( WLSM_M_Role::check_permission( array( 'manage_classes', 'manage_subjects', 'manage_timetable', 'manage_attendance', 'manage_study_materials', 'manage_homework', 'manage_notices', 'manage_student_leaves', 'manage_events' ), $permissions ) ) {
	...
	public static function menu_page_assets() {
		...
		wp_enqueue_style( 'wlsm-print-preview', WLSM_PLUGIN_URL . 'assets/css/print/wlsm-preview.css', array(), '4.7', 'all' );
		wp_enqueue_style( 'wlsm-admin', WLSM_PLUGIN_URL . 'assets/css/wlsm-admin.css', array(), '4.7', 'all' );
		wp_enqueue_style( 'wlsm-school-header', WLSM_PLUGIN_URL . 'assets/css/wlsm-school-header.css', array(), '4.7', 'all' );
		...
		wp_enqueue_script( 'wlsm-admin', WLSM_PLUGIN_URL . 'assets/js/wlsm-admin.js', array( 'jquery', 'jquery-form' ), '4.7', true );
