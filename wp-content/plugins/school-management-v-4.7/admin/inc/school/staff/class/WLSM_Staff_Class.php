	...
	public static function fetch_classes() {
		...
		if ( count( $filter_rows_limit ) ) {
			foreach ( $filter_rows_limit as $row ) {
				...
				$add_section = '<a class="btn btn-sm btn-outline-primary wlsm-font-bold" href="' . esc_url( $page_url . "&action=sections&id=" . $row->ID ) . '">' . esc_html__( 'Manage Sections', 'school-management' ) . '</a>';
				if ( ! $sections_count ) {
					$sections_count = 0;
				} else {
