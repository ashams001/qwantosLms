			...
			try {
				...
				while ( $line = fgetcsv( $csv_file ) ) {
					...
					if ( empty( $admission_date ) ) {
						throw new Exception( esc_html__( 'Please provide admission date.', 'school-management' ) . WLSM_Import::get_csv_error_msg( $row, 3 ) );
					} else {
						$admission_date = $admission_date->format( 'Y-m-d' );
					}
					if ( empty( $admission_number ) ) {
						throw new Exception( esc_html__( 'Please provide admission number.', 'school-management' ) . WLSM_Import::get_csv_error_msg( $row, 2 ) );
					}
