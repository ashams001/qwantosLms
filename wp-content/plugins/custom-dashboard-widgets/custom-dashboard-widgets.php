...
//						SELECT *
//                        FROM wp_posts AS posts_lp_course
//                        WHERE 1=1 AND posts_lp_course.post_type = 'lp_course' "));
//    $items = $item = array();
//    $item['title'] = __('Total Courses');
...
//    $items[] = $item;
    ...
    $res3 = $wpdb->query( $wpdb->prepare( "
                        ...
                        FROM wp_posts AS posts_lp_course
                        WHERE 1=1 
                           AND posts_lp_course.post_status = 'publish'
                           AND posts_lp_course.post_type = 'lp_course' "));
    $item['title'] = __('Active Courses');
    $item['icon'] = 'dashicons-visibility';
    $item['link'] = 'site_url';
    $item['data'] = $res3;
    $item['status'] = 'checked';
    ...
    $res2 = $wpdb->query( $wpdb->prepare( "SELECT * FROM wp_users WHERE 1"));

    $item['title'] = __('Total Users');
    ...
    $res4 = $wpdb->query( $wpdb->prepare( "SELECT * FROM wp_users WHERE 1"));

    $item['title'] = __('Active Users');
    $item['icon'] = 'dashicons-universal-access-alt';
    $item['link'] = 'profile.php';
    $item['status'] = 'checked';
    $item['data'] = $res4;
...
/////////////// Add Custome Widget //////////////////////
function custom_dashboard_count_widget(){
	echo '<link rel="stylesheet" type="text/css" href="'.plugins_url( 'dw_style.css', __FILE__ ).'" />';
   ...
   // echo '<h4>'.__('Data', 'DashboardWidgets').'</h4>';
    ...
    $widget_count_button_class = "main_bashboard_widget_count_button";
...
function custom_dashboard_widget(){
    ...
    echo '<h4>'.__('Course Admin Shortcuts', 'DashboardWidgets').'</h4>';
...
function add_custom_dashboard_widget(){
    ...
    wp_add_dashboard_widget('custom_dashboard_count_widget',__('Dashboard', 'DashboardWidgets'),'custom_dashboard_count_widget','rc_mdm_configure_my_rss_box');
    add_meta_box('custom_dashboard_widget','My comments','custom_dashboard_widget'
    ...
    add_meta_box('custom_dashboard_count_widget','My comments','custom_dashboard_count_widget'
        ,get_current_screen() // Take a look at the output of `get_current_screen()` on your dashboard page
        ,'normal' // Valid: 'side', 'advanced'
        ,'high' // Valid: 'default', 'high', 'low'
    );
