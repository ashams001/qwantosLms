...
<?php foreach ( $instructor as $user_id => $course_id ) { ?>
	...
	$instagram   = get_the_author_meta( 'lp_info_instagram', $course_id->ID );
	$linkein     = get_the_author_meta( 'lp_info_linkedin', $course_id->ID );
				...
				<?php if ( $image_crop ) { ?>
					<a class="avatar" href="<?php echo learn_press_user_profile_link( $course_id->ID ); ?>">
						<img src="<?php echo esc_url( $image_crop ); ?>"
							 alt="<?php echo get_the_author_meta( 'display_name', $course_id->ID ); ?>"/>
					</a>
				<?php } else { ?>
