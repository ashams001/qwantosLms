...
if ( ! class_exists( 'LP_Addon_Announcements' ) ) {
	...
	class LP_Addon_Announcements extends LP_Addon {
		...
		public function __construct() {
			parent::__construct();
			add_action( 'learn-press/register-emails', array( $this, 'add_announcements_emails' ), 10, 2 );
			add_action( 'plugins_loaded', array( $this, 'backward_add_announcements_emails' ), 10 );
		...
		public function frontend_editor_enqueue() {
			...
			wp_enqueue_script( 'lp-announcements-editor-js', LP_ADDON_ANNOUNCEMENTS_URI . 'assets/js/admin.announcements.js', array( 'frontend-course-editor' ), LP_ADDON_ANNOUNCEMENTS_VER, true );
		}
		 ...
		 * Add email classes.
		 */
		public function add_announcements_emails( &$emails, $ojb ) {
			if ( ! $emails ) {
				$emails = LP_Emails::instance()->emails;
			}

			$emails['LP_Email_Announcements'] = include( 'emails/class-lp-email-announcements.php' );
		}

		public function backward_add_announcements_emails() {
			if ( class_exists( 'LP_Emails' ) ) {
				$emails = LP_Emails::instance()->emails;
				$this->add_announcements_emails( $emails, LP_Emails::instance() );
			}
		}
