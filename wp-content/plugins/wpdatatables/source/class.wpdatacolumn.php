...
class WDTColumn {
    ...
    private function getColumnMinValue ()
    {
        ...
        $minQuery = "SELECT MIN(" . $columnOrigHeader . ") as min FROM (" . $parentTable->getTableContent() . ") as parentTable";

        if ($isMySql) {
            $minQuery = "SELECT MIN({$leftSysIdentifier}{$columnOrigHeader}{$rightSysIdentifier}) as min FROM (" . $parentTable->getTableContent() . ") as parentTable";
        } else if ($isMSSql) {
            $minQuery = "SELECT min({$leftSysIdentifier}{$columnOrigHeader}{$rightSysIdentifier}) as min FROM (" . $parentTable->getTableContent() . ") as parentTable";
    ...
    private function getColumnMaxValue ()
    {
        ...
        $maxQuery = "SELECT MAX(" . $columnOrigHeader . ") AS max FROM (" . $parentTable->getTableContent() . ") AS parentTable";

        if ($isMySql) {
            $maxQuery = "SELECT MAX({$leftSysIdentifier}{$columnOrigHeader}{$rightSysIdentifier}) AS max FROM (" . $parentTable->getTableContent() . ") AS parentTable";
        } else if ($isMSSql) {
            $maxQuery = "SELECT max({$leftSysIdentifier}{$columnOrigHeader}{$rightSysIdentifier}) as max FROM (" . $parentTable->getTableContent() . ") as parentTable";
        } else if ($isPostgreSql) {
            $maxQuery = "SELECT MAX({$leftSysIdentifier}{$columnOrigHeader}{$rightSysIdentifier}) AS max FROM (" . $parentTable->getTableContent() . ") AS parentTable";
